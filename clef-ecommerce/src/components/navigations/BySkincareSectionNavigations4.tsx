...
import type { CategoryConfig, SubcategorySlug } from '../../data/category-config';
import { getSubcategoryHref } from '../../data/category-config';

type BySkincareSectionNavigations4Props = {
  categoryConfig: CategoryConfig;
  activeSubcategory: SubcategorySlug;
};

const BySkincareSectionNavigations4: React.FC<BySkincareSectionNavigations4Props> = ({
  categoryConfig,
  activeSubcategory,
}) => {
    return (
  ...
  <nav className="relative">
    <div className="flex w-full items-center justify-end md:justify-center h-14 py-3 px-6 bg-purple-100">
      <div className="hidden md:flex items-center">
        {categoryConfig.subcategories.map((item, index) => (
          <a
            className={`inline-flex items-center text-sm font-bold hover:text-rhino-400 clef-link-highlight ${index < categoryConfig.subcategories.length - 1 ? 'mr-10' : ''} ${item.slug === activeSubcategory ? 'text-purple-500' : 'text-rhino-700'}`}
            href={getSubcategoryHref(categoryConfig.slug, item.slug)}
            key={item.slug}
            aria-current={item.slug === activeSubcategory ? 'page' : undefined}
          >
      ...
      <button className="md:hidden text-coolGray-400 hover:text-coolGray-600 clef-icon-button">
...
export default BySkincareSectionNavigations4;
