  ...
  return (
    <>
      <Link
        ...
        className="relative mr-6 inline-block text-coolGray-400 hover:text-coolGray-600 clef-link-highlight"
        href="/account/favourite"
      >
        {favouriteIds.length > 0 && (
          <span className="absolute right-0 top-0 -mr-2 -mt-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-orange-500 text-xs font-bold text-white">
            {favouriteIds.length}
          </span>
        )}
      ...
      <Link
        ...
        href={isCustomerLoading ? '/login' : accountHref}
      >
        <AccountIcon />
      </Link>
    </>
